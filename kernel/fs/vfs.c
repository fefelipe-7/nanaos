...
void vfs_init(void) {
    ...
    if ( (size_t)(&hello_elf_len) != 0 ) {
        /* hello_elf and hello_elf_len are provided by the generated header */
        if ((size_t)hello_elf_len > 0) {
            vfs_create("/bin", 1, vfs_root);
            vfs_write_file("/bin/hello", (const char *)hello_elf, (size_t)hello_elf_len, vfs_root);
        }
    }
